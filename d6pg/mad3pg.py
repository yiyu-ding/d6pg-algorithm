...
def evaluate(env_id, dtdemaddpg, n_episode=10, episode_length=40):
    env = make_env(env_id)
    returns = np.zeros(len(env.agents))
    for _ in range(n_episode):
        obs = env.reset()
        for t_i in range(episode_length):
            actions = dtdemaddpg.take_action(obs, explore=False)
...
def runoneturn():
    ...
    dtdemaddpg = DTDEMADDPG(env, device, actor_lr, critic_lr, hidden_dim, state_dims,
    ...
    for i_episode in range(num_episodes):
        ...
        for e_i in range(episode_length):
            ...
            actions = dtdemaddpg.take_action_adj(state_adj, explore=True)
            ...
            if (judge == len(env.agents)) and (total_step % update_interval == 0):
                ...
                dtdemaddpg.update(sample)

                dtdemaddpg.update_all_targets()

        if (i_episode + 1) % 100 == 0:
            ep_returns = evaluate(env_id, dtdemaddpg, n_episode=100)
            return_list.append(ep_returns)
            print(f"Episode: {i_episode+1}, {ep_returns}")
            #####################################################################
            # if (i_episode + 1) / 100 == 50:
            #     #env.showjudge = True
            #     demonstrate(env_id, dtdemaddpg, n_episode=1)
            #     #env.showjudge = False
            if (i_episode + 1) / 100 == 50:
                demonstrate(env_id, dtdemaddpg, n_episode=1)
...
if __name__ == '__main__':
    ...
    #     np.savetxt('./result/uav-8/local_mad3pg.csv', turn_array, fmt='%f', delimiter=',')
    # if env_id == "simple_air_global":
    #     np.savetxt('./result/uav-8/global_mad3pg.csv', turn_array, fmt='%f', delimiter=',')
